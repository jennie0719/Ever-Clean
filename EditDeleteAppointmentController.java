...
public class EditDeleteAppointmentController implements Initializable {
    ...
    void delete(ActionEvent event) throws IOException {
       ...
       while (allCorrect){ 
        ...
        Appointment a = appointment_tableview.getSelectionModel().getSelectedItem();

        for (int j = 0; j < ReadData.appointmentArrayList.size(); j++) {
            if (ReadData.appointmentArrayList.get(j).getAppointmentID().equals(a.getAppointmentID())) {
        ...
        appointment_tableview.setItems(list);
          ...
          //display reminder if no appointment is selected
          reminder.setText("Please choose what to edit/delete");
    ...
    void edit(ActionEvent event) throws IOException {
      ...
      while (allCorrect){ 
        try{
        //Move to Edit Appointment page after an appointment is selected
    ...
    void search(ActionEvent event) {
      //search an appointment by Appointment ID, e.g. A3
    ...
    @FXML
    //only show appointments of Cleaners under that Manager's team
    public void initialize(URL url, ResourceBundle rb) {
        for (int j = 0; j < ReadData.appointmentArrayList.size(); j++) {
            for (int i = 0; i < ReadData.cleanerArrayList.size(); i++) {
                if (ReadData.cleanerArrayList.get(i).getCleanerID().equals(ReadData.appointmentArrayList.get(j).getAppointmentStaff())) {
                    if (ReadData.cleanerArrayList.get(i).getCleanerTeam().equals(ReadData.userteam)) {
                        list.add(ReadData.appointmentArrayList.get(j));
                    } else {
                        continue;
                    }
